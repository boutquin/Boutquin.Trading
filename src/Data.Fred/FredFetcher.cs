...
public sealed class FredFetcher : IEconomicDataFetcher, IDisposable
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="FredFetcher"/> class.
    /// </summary>
    /// <param name="apiKey">The FRED API key.</param>
    /// <param name="httpClient">Optional HTTP client for making requests.</param>
    /// <param name="apiEndpoint">Base URL for the FRED API.</param>
    public FredFetcher(
        string apiKey,
    ...
    {
        var url = $"{_apiEndpoint}/fred/series/observations?series_id={Uri.EscapeDataString(seriesId)}&api_key={Uri.EscapeDataString(_apiKey)}&file_type=json";
