...
public sealed class Portfolio : IPortfolio
{
    ...
    /// <inheritdoc />
    public bool EnableDividendReinvestment { get; }
    ...
    public Portfolio(
        ...
        bool isLive = false,
        bool enableDividendReinvestment = false)
    {
        // Validate parameters
        ...
        IsLive = isLive;
        EnableDividendReinvestment = enableDividendReinvestment;
    ...
    public void UpdateCashForDividend(Asset asset, decimal dividendPerShare, decimal currentPrice = 0m)
    {
        ...
        foreach (var strategyEntry in Strategies)
        {
            ...
            // DRIP: reinvest dividends into whole shares at current Close price.
            // Note: uses MidpointRounding.ToZero (truncation) instead of the project-standard
            // AwayFromZero because DRIP must never overspend — rounding up could buy more shares
            // than the dividend covers. Assumes currentPrice is in the asset's native currency
            // (same as dividendPerShare), which holds for domestic assets and standard data providers.
            if (EnableDividendReinvestment && currentPrice > 0)
            {
                var sharesToReinvest = (int)Math.Round(dividendAmount / currentPrice, MidpointRounding.ToZero);
                if (sharesToReinvest > 0)
                {
                    var reinvestmentCost = sharesToReinvest * currentPrice;
    ...
    /// Delegates to the internal brokerage's ProcessPendingOrdersAsync.
    /// </summary>
    ...
    {
        _activeCancellationToken = cancellationToken;
        await _broker.ProcessPendingOrdersAsync(date, dayData, cancellationToken).ConfigureAwait(false);
    }
