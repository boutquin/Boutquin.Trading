...
public sealed class RiskManager : IRiskManager
{
    ...
    /// <inheritdoc />
    public BatchRiskEvaluation EvaluateBatch(IReadOnlyList<Order> orders, IPortfolio portfolio)
    {
        ...
        if (orders.Count == 0)
        {
            return BatchRiskEvaluation.Allowed;
        }

        var rejections = new List<OrderRiskResult>();

        foreach (var rule in _rules)
        {
            var result = rule.EvaluateBatch(orders, portfolio);
            if (!result.IsAllowed)
            {
                return BatchRiskEvaluation.Rejected(
                    $"[{rule.Name}] {result.RejectionReason}");
            }
        }
