...
public static class DrawdownAnalyzer
{
        ...
        SortedDictionary<DateOnly, decimal> equityCurve)
    {
        ...
        for (var i = 1; i < entries.Count; i++)
        {
            ...
            if (value >= peak)
            {
                ...
                if (inDrawdown)
                {
                    ...
                    // Use entry index differences for trading-day duration (per CLAUDE.md convention)
                    var durationDays = i - peakIndex;
                    var recoveryDays = i - troughIndex;
        ...
        {
            var depth = (troughValue - peak) / peak;
            var durationDays = entries.Count - 1 - peakIndex;
