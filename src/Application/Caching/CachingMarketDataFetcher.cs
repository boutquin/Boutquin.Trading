...
public sealed class CachingMarketDataFetcher : IMarketDataFetcher, IDisposable
{
    ...
    /// <param name="inner">The underlying market data fetcher.</param>
    ...
    {
        // Materialize to List before building key or creating Lazy to avoid double-enumeration of non-replayable IEnumerable
        var symbolList = symbols.ToList();
        var key = BuildMarketDataKey(symbolList);
        var lazy = _marketDataCache.GetOrAdd(key, _ => new Lazy<Task<List<KeyValuePair<DateOnly, SortedDictionary<Asset, MarketData>>>>>(
            () => MaterializeMarketDataAsync(symbolList, CancellationToken.None)));

        List<KeyValuePair<DateOnly, SortedDictionary<Asset, MarketData>>> data;
        try
        {
        ...
        catch
        {
            ...
            _marketDataCache.TryRemove(KeyValuePair.Create(key, lazy));
            throw;
        }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var key = BuildFxKey(pairList);
        var lazy = _fxCache.GetOrAdd(key, _ => new Lazy<Task<List<KeyValuePair<DateOnly, SortedDictionary<CurrencyCode, decimal>>>>>(
            () => MaterializeFxRatesAsync(pairList, CancellationToken.None)));

        List<KeyValuePair<DateOnly, SortedDictionary<CurrencyCode, decimal>>> data;
        try
        {
            data = await lazy.Value.ConfigureAwait(false);
        }
        ...
        {
            // Evict the faulted entry so the next caller retries; only remove our exact Lazy instance
            _fxCache.TryRemove(KeyValuePair.Create(key, lazy));
            throw;
        }
