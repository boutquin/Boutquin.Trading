...
public sealed class WriteThroughMarketDataFetcher : IMarketDataFetcher, IDisposable
{
    ...
    /// <summary>Initializes a new instance with the specified fetcher, data directory, and logger.</summary>
    /// <param name="apiFetcher">The underlying API fetcher.</param>
    /// <param name="dataDirectory">The directory for CSV cache files.</param>
    ...
    /// <summary>Initializes a new instance with the specified fetcher and data directory.</summary>
    /// <param name="apiFetcher">The underlying API fetcher.</param>
    /// <param name="dataDirectory">The directory for CSV cache files.</param>
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        if (uncached.Count > 0)
        {
            ...
            {
                _logger.LogError(ex, "API fetch failed for {Count} symbols; CSV cache may be incomplete", uncached.Count);
                throw;
            }
        }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        if (uncached.Count > 0)
        {
            ...
            List<KeyValuePair<DateOnly, SortedDictionary<CurrencyCode, decimal>>> apiData;
            try
            {
                // Materialize API data per pair and write CSV
                apiData = new List<KeyValuePair<DateOnly, SortedDictionary<CurrencyCode, decimal>>>();
            ...
            {
                _logger.LogError(ex, "API fetch failed for {Count} FX pairs; CSV cache may be incomplete", uncached.Count);
                throw;
            }
