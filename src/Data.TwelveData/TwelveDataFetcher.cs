...
public sealed class TwelveDataFetcher : IMarketDataFetcher, IDisposable
{
    ...
    /// <param name="exchange">Optional exchange filter (e.g., "TSX"). When set, appended to all API requests to disambiguate symbols listed on multiple exchanges.</param>
    ...
    /// <summary>Returns "&amp;exchange=XXX" when an exchange filter is configured, empty string otherwise.</summary>
    private string ExchangeParam() =>
        string.IsNullOrWhiteSpace(_exchange) ? "" : $"&exchange={Uri.EscapeDataString(_exchange)}";
    ...
    {
        var url = $"{_apiEndpoint}/time_series?symbol={escapedTicker}&interval=1day&outputsize=5000{ExchangeParam()}&apikey={_apiKey}";
    ...
    {
        var url = $"{_apiEndpoint}/dividends?symbol={escapedTicker}{ExchangeParam()}&apikey={_apiKey}";
    ...
    {
        var url = $"{_apiEndpoint}/splits?symbol={escapedTicker}{ExchangeParam()}&apikey={_apiKey}";
