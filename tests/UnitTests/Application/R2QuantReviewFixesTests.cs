...
public sealed class R2QuantReviewFixesTests
{
    ...
    public void R2Q02_ComputeTargetWeights_ConvergesToOptimal_TwoAsset()
    {
        ...
        // Higher-return asset should get at least as much weight.
        // Analytical solver may produce exactly equal weights when returns
        // are symmetric with equal variance structure.
        weights[a].Should().BeGreaterThanOrEqualTo(weights[b]);
        weights.Values.Sum().Should().BeApproximately(1.0m, 1e-8m);
    }
    ...
    public void R2Q06_Generate_DifferentDateRanges_AlignsByDate()
    {
        ...
        html.Should().Contain("Information Ratio", "Should contain information ratio");
    }
