...
public sealed class DependencyInjectionTests
{
    ...
    public void BacktestOptions_ShouldLoadFromConfig()
    {
        ...
        options.StartDate.Should().Be("2020-01-01");
        options.EndDate.Should().Be("2025-12-31");
        options.BaseCurrency.Should().Be("USD");
        options.RebalancingFrequency.Should().Be("Monthly");
    ...
    public void BacktestOptions_ShouldRespectOverrides()
    {
        var sp = BuildServiceProvider(new Dictionary<string, string?>
        {
            ["Backtest:StartDate"] = "2015-06-01",
            ["Backtest:BaseCurrency"] = "EUR",
        });

        var options = sp.GetRequiredService<IOptions<BacktestOptions>>().Value;
        options.StartDate.Should().Be("2015-06-01");
        options.BaseCurrency.Should().Be("EUR");
    }
