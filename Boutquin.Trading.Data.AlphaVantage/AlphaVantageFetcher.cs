...
public sealed class AlphaVantageFetcher : IMarketDataFetcher
{
    ...
    /// <example>
    /// <code>
    /// var assets = new List&lt;string&gt; { new Asset("AAPL"), new Asset("GOOG") };
    ...
    public async IAsyncEnumerable<KeyValuePair<DateOnly, SortedDictionary<Asset, MarketData>?>> FetchMarketDataAsync(IEnumerable<Asset> assets)
    {
        ...
        for (var i = 0; i < assets.Count(); i++)
        {
            ...
            if (cachedData != null)
            {
                await foreach (var dataPoint in DeserializeAndYieldCachedMarketData(cachedData, asset, options).ConfigureAwait(false))
                {
    ...
    public async IAsyncEnumerable<KeyValuePair<DateOnly, SortedDictionary<CurrencyCode, decimal>>> FetchFxRatesAsync(IEnumerable<string> currencyPairs)
    {
        ...
        for (var i = 0; i < currencyPairs.Count(); i++)
        {
            ...
            if (cachedData != null)
            {
                await foreach (var dataPoint in DeserializeAndYieldCachedFxRates(cachedData, pair, options).ConfigureAwait(false))
                {
    ...
    {
        return $"{_apiEndpoint}?function=TIME_SERIES_DAILY_ADJUSTED&symbol={asset.Ticker}&apikey={_apiKey}&outputsize=full&datatype=json";
    }
