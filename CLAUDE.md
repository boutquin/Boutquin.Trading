...
- **Stop orders use High/Low for trigger evaluation** — `SimulatedBrokerage.HandleStopOrder` checks `marketData.High >= stopPrice` (buy) and `marketData.Low <= stopPrice` (sell), not Close. This correctly models intraday stop triggering.
